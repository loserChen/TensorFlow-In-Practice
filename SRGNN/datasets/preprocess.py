...
#生成每个session id对应日期的字典,以及对每个session里点击的item按顺序进行排序
...
sorted_counts = sorted(iid_counts.items(), key=operator.itemgetter(1))      #根据出现次数进行排序

length = len(sess_clicks)
for s in list(sess_clicks): #遍历每个id
    curseq = sess_clicks[s]
    filseq = list(filter(lambda i: iid_counts[i] >= 5, curseq))    #过滤出item出现次数大于等于5次的
    if len(filseq) < 2:
        del sess_clicks[s]
        del sess_date[s]
    else:
...
for _, date in dates:   #找出最大的日期
    if maxdate < date:
        maxdate = date

# 利用7天来当测试集，即将maxdate-7得到所谓划分点
splitdate = 0
if opt.dataset == 'yoochoose':
